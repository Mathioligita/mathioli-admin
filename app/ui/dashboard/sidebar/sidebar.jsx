...
const Sidebar = () => {
  const router = useRouter();
  const pathname = window.location.pathname.split("/")[2]; // Get current pathname
  ...
  const model = [
    ...
    },
    {
      ...
      items: [
        ...
        { label: "Book", icon: "fa-solid fa-book", to: "/dashboard/book" },
        {
          label: "Quote",
          icon: "fa-solid fa-quote-left",
          to: "/dashboard/quote",
        ...
        {model.map((section, index) => (
          <li key={index} className="layout-root-menuitem">
            {section.items && (
              <ul className="sub-menu">
                {section.items.map((item, subIndex) => {
                  const isActive = pathname === item.to.split("/")[2]; // Check if the current route matches
                  return (
                    <a
                      href={item.to}
