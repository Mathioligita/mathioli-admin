...
import { CategoryAPI, companyGET, companyPOST, PlanGET } from "../../../api/page";
...
export default function Page() {
  ...
  const fetchIndustries = async () => {
    ...
    try {
      ...
      const industryData = response?.data?.map((industry) => ({
  ...
  const fetchLeads = async () => {
    ...
    try {
      ...
      if (response) {
        ...
        setNodes(response?.data?.company);
      }
    } catch (error) {
      setError(
        error.response?.data?.message || "An unexpected error occurred."
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const response = await companyPOST(data);
      if (response) {
